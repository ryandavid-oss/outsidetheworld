...
| Regenerative boss retry | Loaded Wound `qa=retry`: checkpoint stored one lost heart, QA forced a fatal hit, then clicked Retry | Fresh boss attempt begins with five hearts while checkpoint progress remains | Retry showed five hearts, retained checkpoint hit marker `1`, restored clock `06:00`, score `2400`, rifle, and a reset 50-health boss | Pass | Approved design follow-up | Regenerative Wound reset | Pass |
