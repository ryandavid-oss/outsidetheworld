...
- Public forms write only to the `otw-private-intake` Cloudflare Worker and its private
  `otw-private-intake` D1 database.
...
4. Search all four D1 intake tables using normalized contact information. Keep personal values out
   of shell history, source files, tickets, and logs; use a restricted temporary SQL file or the
...
6. For correction, update only the confirmed matching records. For deletion or unsubscribe, remove
   matching records from every applicable intake table; do not merely hide them in the UI.
