...
`,root+'/cfm906-teens.html'],{encoding:'utf8'});
const closeEvents=[];
let doc;
class E {
...
jump('psalm');
const reflections=doc.querySelectorAll('[data-reflection]');
assert.equal(reflections.length,3);
for (const choice of reflections) {
 choice.fire('click');
 assert.equal(reflections.filter(c=>c.attrs['aria-pressed']==='true').length,1);
 assert.ok(get('psalm-feedback').textContent.length>30);
 assert.equal(choice.disabled,false,'reflection choices remain available without scoring');
}
select('[data-reflection="lamp"]').fire('click');assert.match(get('psalm-feedback').textContent,/out of sight/);
click('psalm-reset');assert.equal(reflections.filter(c=>c.attrs['aria-pressed']==='true').length,0);assert.equal(get('psalm-feedback').textContent,'Choose a phrase to talk about.');
...
playback(video,187.8);assert.equal(get('y-cue').open,false);playback(video,188.05);assert.equal(get('y-cue').open,true);assert.equal(video.paused,true);assert.match(get('cue-title').textContent,/willingness to forgive/);assert.equal(get('cue-time').textContent,'0:30');
...
playback(video,440.4);assert.equal(get('y-cue').open,false);playback(video,440.7);assert.equal(get('y-cue').open,true);assert.match(get('cue-title').textContent,/participate/);click('cue-close');assert.equal(video.paused,true);assert.equal(video.playCount,1,'closing does not resume');
...
playback(learning,153.1);assert.equal(get('y-cue').open,false);playback(learning,153.3);assert.equal(get('y-cue').open,true);assert.match(get('cue-title').textContent,/you’d like to explore/);assert.equal(get('cue-time').textContent,'0:45');
...
jump('discuss');const discussion=select('[data-short-timer]');discussion.fire('click');now+=12000;tick();assert.equal(get('discuss-countdown').textContent,'1:03');jump('finish');now+=60000;tick();assert.equal(get('discuss-countdown').textContent,'1:15');
...
console.log('PASS: source markup, navigation, lazy media, keyboard guards, warmup choices, discussion prompts, scripture reflection/modal, preview focus, all three checkpoints, one-shot pauses, seeking, guided toggle, local files/errors, independent timers, fullscreen controls.');
