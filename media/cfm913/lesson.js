(() => {
  ...
  function renderDiscussion() {
    ...
    let body = '';
    if (slide === 'opening') body = `<p class="eyebrow">AS WE GATHER · PROVERBS &amp; ECCLESIASTES</p><h2 class="opening-title" id="slide-title" tabindex="-1">Wisdom for<br>the <em>hard days.</em></h2><p class="opening-class-question">What verse, question, or thought did you bring?</p><p class="slide-gentle">You’re welcome to share, or just listen.</p><details class="opening-context"><summary>From this week’s Come, Follow Me</summary><blockquote>Life is always better—if not always perfect—when we trust and follow the Lord Jesus Christ.</blockquote>${external(OFFICIAL, 'Open the official lesson')}</details>`;
    else if (slide === 'closing') body = `<p class="eyebrow">CARRY IT INTO THE WEEK</p><h2 class="opening-title" id="slide-title" tabindex="-1">One verse.<br>One <em>next step.</em></h2><p class="opening-class-question">What will you do this week to follow Jesus Christ?</p>${external(OFFICIAL, 'Continue the official Come, Follow Me study', 'class-source')}`;
    ...
    else body = `<p class="eyebrow">${escape(s.title)} · REFLECT TOGETHER</p><h2 class="discussion-big-question" id="slide-title" tabindex="-1">${escape(s.questions[questionIndex])}</h2><p class="slide-gentle">Take a moment to think. Then let’s talk.</p><div class="class-question-actions"><button type="button" class="class-source" data-action="another-class">Another question ↻</button><button type="button" class="class-source" data-read="${s.passage}">Return to the passage ↗</button><button type="button" class="class-source" data-action="christ">Consider the Savior ↗</button></div>`;
